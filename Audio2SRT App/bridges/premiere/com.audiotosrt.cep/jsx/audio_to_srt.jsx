...
/* ── BETA: ripple-cut silent ranges from the live timeline via QE DOM ──────────
 * csv = "start:end,start:end,..." in SEQUENCE seconds.
 * QE DOM is undocumented and version-dependent; everything is wrapped in
 * try/catch and the panel always falls back to the tightened-clip export.
 * Cuts are applied back-to-front so earlier timecodes don't shift. */
var TICKS_PER_SEC = 254016000000;

function _seqFps(seq) {
    try {
        if (seq.timebase) return Math.round(TICKS_PER_SEC / Number(seq.timebase));
    } catch (e) {}
    return 25;
}

function _tc(seconds, fps) {
    var f = Math.round(seconds * fps);
    var ff = f % fps;
    var s = Math.floor(f / fps);
    var sec = s % 60, mn = Math.floor(s / 60) % 60, hr = Math.floor(s / 3600);
    function p(n) { return (n < 10 ? "0" : "") + n; }
    return p(hr) + ":" + p(mn) + ":" + p(sec) + ":" + p(ff);
}

function applyRippleCuts(csv) {
    try {
        var seq = app.project ? app.project.activeSequence : null;
        if (!seq) return _err("No active sequence.");
        app.enableQE();
        var qeSeq = qe.project.getActiveSequence();
        if (!qeSeq) return _err("QE sequence unavailable in this version.");
        ...
        for (var i = 0; i < parts.length; i++) {
            ...
            if (ab.length === 2) ranges.push([parseFloat(ab[0]), parseFloat(ab[1])]);
        }
        ranges.sort(function (a, b) { return b[0] - a[0]; });  // back-to-front

        var removed = 0;
        for (var j = 0; j < ranges.length; j++) {
            var s = ranges[j][0], e = ranges[j][1];
            var tcS = _tc(s, fps), tcE = _tc(e, fps);
            try { qeSeq.razor(tcS); } catch (e1) {}
            try { qeSeq.razor(tcE); } catch (e2) {}

            // Ripple-delete the segment now sitting at [s,e] on each audio track.
            var did = false;
            try {
                var nAud = qeSeq.numAudioTracks;
                for (var a = 0; a < nAud; a++) {
                    var tr = qeSeq.getAudioTrackAt(a);
                    if (!tr) continue;
                    var n = tr.numItems;
                    for (var k = n - 1; k >= 0; k--) {
                        var it = tr.getItemAt(k);
                        if (!it) continue;
                        var st = 0;
                        try { st = Number(it.start.ticks) / TICKS_PER_SEC; } catch (e3) { continue; }
                        if (st >= s - 0.02 && st < e - 0.01) {
                            try { it.remove(true, true); did = true; } catch (e4) {}
                        }
                    }
                }
            } catch (e5) {}
        ...
        return _err("QE ripple-cut failed: " + e.toString());
    }
}
